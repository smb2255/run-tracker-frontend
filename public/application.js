...
var createNewRun = function createNewRun(data) {
  return $.ajax({
...
var showAllMyRuns = function showAllMyRuns() {
  return $.ajax({
    url: config.apiOrigin + '/runs/',
    method: 'GET',
    headers: {
...
var userEvents = __webpack_require__(34);
...
var onNewRun = function onNewRun(event) {
  event.preventDefault();
  console.log('onNewRun reached');
  var data = getFormFields(event.target);
  data = JSON.stringify(data);
  api.createNewRun(data).then(ui.newRunSuccess).catch(ui.newRunFailure);
};

var onShowAllMyRuns = function onShowAllMyRuns(event) {
  event.preventDefault();
  console.log('onShowAllMyRuns reached');
  api.showAllMyRuns().then(ui.showAllMyRunsSuccess).catch(ui.showAllMyRunsFailure);
};
...
var onDeleteRun = function onDeleteRun(event) {
  ...
  api.deleteRun(data.run.id).then(ui.deleteRunSuccess).catch(ui.deleteRunFailure);
...
var store = __webpack_require__(3);
var api = __webpack_require__(4);
var showAllMyRunsTemplate = __webpack_require__(16);
// const listHtml = require('./events')

var newRunSuccess = function newRunSuccess(data) {
  var runHtml = '<H2>Run has been logged!</H2>\n    <ul>\n    <li>TIME: ' + data.run.time + '</li>\n    <LI>DISTANCE: ' + data.run.distance + '</LI>\n    <LI>DATE: ' + data.run.date + '</LI>\n    </ul>\n    ';
  $('#user-messages').html(runHtml);
  $('#show-all-runs-title').show();
  $('#delete-runs-message').html('');
  $('#update-run-message').html('');
  // $('#new-run-form').reset()
  api.showAllMyRuns().then(showAllMyRunsSuccess).catch(showAllMyRunsFailure);
};
var newRunFailure = function newRunFailure() {
...
var showAllMyRunsSuccess = function showAllMyRunsSuccess(data) {
  ...
  console.log(data);
  var showAllRunsHtml = showAllMyRunsTemplate({ runs: data.runs });
...
var showAllMyRunsFailure = function showAllMyRunsFailure() {
...
var deleteRunFailure = function deleteRunFailure(id) {
...
module.exports = {
  newRunSuccess: newRunSuccess,
  newRunFailure: newRunFailure,
  showAllMyRunsSuccess: showAllMyRunsSuccess,
  showAllMyRunsFailure: showAllMyRunsFailure,
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "<section id=\"show-all-runs\">\n  <div data-id=\""
...
var getFormFields = __webpack_require__(7);
var api = __webpack_require__(4);
var userUi = __webpack_require__(35);
...
var onSignUp = function onSignUp(event) {
  ...
  api.signUp(data).then(userUi.signUpSuccess).catch(userUi.signUpFailure);
...
var onSignIn = function onSignIn(event) {
  ...
  api.signIn(data).then(userUi.signInSuccess).catch(userUi.signInFailure);
};
var onChangePass = function onChangePass(event) {
  var data = getFormFields(event.target);
  console.log('onChangePass reached');
  event.preventDefault();
  api.changePass(data).then(userUi.changePassSuccess).catch(userUi.changePassFailure);
};
var onSignOut = function onSignOut(event) {
  event.preventDefault();
  console.log('onSignOut reached');
  api.signOut().then(userUi.signOutSuccess).catch(userUi.signOutFailure);
};
